...
export default function getApp(isSimulatingNetwork) {
  ...
  function RealSidebar() {
    ...
    return (
      ...
      >
        <h1>Sidebar ({didHydrate ? "Hydrated with JS" : "Initial HTML"})</h1>
  ...
  function RealContent() {
    ...
    return (
      ...
      >
        <h1>Content ({didHydrate ? "Hydrated with JS" : "Initial HTML"})</h1>
        <button onClick={() => setCount((c) => c + 1)}>
  ...
  const Effector = ({ loop }) => {
    // 🚨🚨🚨🚨 COMMENT THIS LINE AND THINGS WORK
  ...
  function App() {
    ...
    const onFirstHydration = useCallback(() => {
      setState({ foo: "baz" });
    });

    return (
      <>
        <Effector loop={onFirstHydration} />
        <ContextProvider value={state}>
          ...
          >
            ...
            <h2>
              This app is server-rendered to HTML. Concurrent Mode lets us
              hydrate parts of UI without waiting for <i>all</i> JS to load.
            </h2>
